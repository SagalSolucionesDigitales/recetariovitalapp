    ...
    <>
      <header className="bg-primary text-primary-foreground">
        <div className="mx-auto max-w-2xl px-5 pb-6 pt-5">
          <div className="flex items-center justify-between">
            ...
            <Link
              to="/cuenta"
              className="grid h-9 w-9 place-items-center rounded-full border border-white/25 bg-white/15 text-xs font-medium"
            >
              {initials}
            </Link>
          </div>
          <h1 className="mt-5 font-serif text-2xl" suppressHydrationWarning>
            {useGreeting()}, {profile?.nombre ?? "👋"}
          </h1>
      ...
      <main className="mx-auto max-w-2xl space-y-3.5 px-5 py-5">
        ...
        {todayCheckin ? (
          ...
          <Link
            to="/progreso"
            className="flex items-center justify-between rounded-2xl bg-accent p-4 text-accent-foreground"
          >
            <div>
        ...
        <section>
          ...
          <div className="overflow-hidden rounded-2xl border border-border bg-card">
            <div className="flex items-center justify-between bg-primary-soft px-4 py-2.5 text-xs">
              ...
              <Link to="/plan" className="font-medium text-primary">
                Ver semana completa →
              </Link>
            </div>
            <div className="divide-y divide-border">
              <MealRow
                icon={Coffee}
                label="Desayuno"
                name={todayPlan?.desayuno?.nombre ?? "Aún no generado"}
              />
              <MealRow
                icon={Sun}
                label="Almuerzo"
                name={todayPlan?.almuerzo?.nombre ?? "Aún no generado"}
              />
              <MealRow
                icon={Moon}
                label="Cena"
            ...
            {!plan && (
              <div className="border-t border-border p-3">
                <Link
                  to="/plan"
                  className="block w-full rounded-lg bg-primary px-4 py-2.5 text-center text-xs font-medium text-primary-foreground"
                >
        ...
        <section>
          ...
          <div className="rounded-2xl border border-border bg-card p-4">
            <div className="flex items-start gap-3">
              <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-primary font-serif text-white">
                C
              </div>
              <div className="flex-1 rounded-2xl rounded-tl-none bg-primary-soft p-3">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-primary">
                  Camila — Coach de nutrición
                </p>
...
function computeStreak(checkins: Array<{ fecha: string; siguio_plan: string | null }>): number {
  const set = new Set(
    checkins
      .filter((c) => c.siguio_plan === "si" || c.siguio_plan === "parcial")
      .map((c) => c.fecha),
  );
  let n = 0;
  const d = new Date();
