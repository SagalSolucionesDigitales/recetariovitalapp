...
function PlanPage() {
  ...
  const [open, setOpen] = useState<"desayuno" | "almuerzo" | "cena" | "postre" | null>(null);

  const mut = useMutation({
    mutationFn: () => gen(),
    onSuccess: () => {
      toast.success("Plan generado");
      qc.invalidateQueries({ queryKey: ["plan-latest"] });
    },
    ...
    <>
      ...
      <main className="mx-auto max-w-2xl space-y-3 px-5 py-5">
        {plan && (
          <Link
            to="/compras"
            className="flex items-center gap-3 rounded-2xl bg-accent-soft p-4 text-accent"
          >
        ...
        {!isLoading && !plan && (
          <div className="rounded-2xl border border-border bg-card p-6 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Genera tu primer plan personalizado en segundos.
            </p>
            <button
              onClick={() => mut.mutate()}
              disabled={mut.isPending}
        ...
        {dia &&
          (["desayuno", "almuerzo", "cena", "postre"] as const).map((key) => {
            const meal = dia[key];
            const Icon =
              key === "desayuno" ? Coffee : key === "almuerzo" ? Sun : key === "cena" ? Moon : Cake;
            const time =
              key === "desayuno"
                ? "8:00 am"
                : key === "almuerzo"
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 flex gap-1.5">
                      <span className="rounded-full bg-primary-soft px-2 py-0.5 text-[10px] font-semibold text-primary">
                        IG {meal?.ig_nivel ?? "Bajo"}
                      </span>
                      {typeof meal?.costo_usd === "number" && (
                        <span className="rounded-full bg-accent-soft px-2 py-0.5 text-[10px] font-semibold text-accent">
                          ${meal.costo_usd.toFixed(2)} USD
                        </span>
                      )}
                    </div>
                  </div>
